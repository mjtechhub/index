...
(function() {
    ...
    function analyzeObservations(obs) {
        ...
        if (gatewayPing === 'no') {
            if (connectionType === 'wifi') {
                ...
                confidence = 'Likely';
                explanation = 'The client workstation cannot reach its local default gateway across the wireless medium. Packets are not traversing the local subnet, pointing to an RF disconnect, incorrect WPA key, AP isolation, or stale DHCP assignment.';
                nextSteps = [
                ...
                confidence = 'Likely';
                explanation = 'The VPN tunnel endpoint or virtual adapter cannot reach the configured tunnel gateway. This commonly indicates tunnel establishment failure, expired credentials, or an interface metric routing conflict.';
                nextSteps = [
                ...
                confidence = 'Likely';
                explanation = 'Failing to ping the local default gateway indicates a breakdown at the Data Link or Physical layer (Layer 1/2), or an invalid static IP/subnet mask configuration on the local host.';
                nextSteps = [
        ...
        // Scenario 2: Gateway OK, but Public IP Ping Fails
            ...
            confidence = 'Likely';
            explanation = 'The local LAN segment and default gateway are responding normally, but outbound packets cannot reach public Internet IP addresses. The failure resides at the edge router, NAT gateway, WAN firewall, or ISP circuit.';
            nextSteps = [
        ...
        // Scenario 3: Gateway & Public IP OK, but DNS Resolution Fails
            ...
            confidence = 'Likely';
            explanation = 'Layer 1 through Layer 3 IP connectivity is fully functional across the Internet, but application hostnames cannot be resolved. The fault is isolated to DNS client configuration, unresponsive recursive DNS resolvers, or firewall filtering on UDP port 53.';
            nextSteps = [
        ...
        // Scenario 4: Gateway, Public IP & DNS OK, but Port Unreachable
            ...
            confidence = 'Likely';
            explanation = 'Network routing and DNS resolution succeed, but the destination TCP port refused the connection or timed out. This isolates the fault to Layer 4/7: perimeter or host firewalls, cloud Security Groups, or the backend service daemon is stopped/bound to loopback.';
            nextSteps = [
        ...
        // Scenario 5: All Tests Succeeded
            ...
            confidence = 'Likely';
            explanation = 'All network infrastructure layers (Physical, Data Link, Network, Transport, and DNS) are healthy and passing packets. If users still experience errors, the issue is at the application layer (HTTP 500 error, TLS certificate expired, CORS policy, or authentication/SSO failure).';
            nextSteps = [
        ...
        // General Inconclusive State
            ...
            confidence = 'Possible';
            explanation = 'The supplied observations do not conclusively isolate a single layer. Complete the untested checks (especially default gateway reachability and DNS resolution) to narrow down the fault domain.';
            nextSteps = [
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function runAnalysis() {
            ...
            confidenceEl.textContent = `${result.confidence} Fault Domain`;
            confidenceEl.className = 'badge ' + (result.confidence === 'Likely' ? 'badge-primary' : 'badge-warning');
