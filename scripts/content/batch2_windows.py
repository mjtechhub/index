...
<ul>
    <li><strong>The Windows Event Log Service (<code>EventLog</code>):</strong> The actual Windows service name is <code>EventLog</code> (display name: <em>Windows Event Log</em>). It runs inside a <code>svchost.exe</code> host process powered by the service DLL <code>wevtsvc.dll</code> (which implements the service functionality, distinguished from the service name itself). The service receives log writes from user-mode APIs and kernel-mode ETW (Event Tracing for Windows) providers.</li>
